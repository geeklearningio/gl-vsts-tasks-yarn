...
export class ProvenanceHelper {
  ...
  public static async GetSessionId(
    ...
    let shouldCreateSession =
      publishPackageMetadata && publishPackageMetadata.toLowerCase() == "true";
    if (shouldCreateSession) {
      const useSessionEnabled = tl.getVariable("Packaging.SavePublishMetadata");
      shouldCreateSession =
        shouldCreateSession &&
        !(useSessionEnabled && useSessionEnabled.toLowerCase() == "false");
    }
    if (shouldCreateSession) {
      tl.debug(
        "Creating provenance session to save pipeline metadata. This can be disabled in the task settings, or by setting build variable Packaging.SavePublishMetadata to false"
      );
      const prov = new ProvenanceApi(baseUrl, handlers, options);
      const sessionRequest = ProvenanceHelper.CreateSessionRequest(feedId);
      try {
        const session = await prov.createSession(
          sessionRequest,
          protocol,
          project
        );
        ...
        tl.warning(
          tl.loc("Warning_SessionCreationFailed", JSON.stringify(error))
        );
      }
    }
    return feedId;
  ...
  public async createSession(
    ...
    return new Promise<SessionResponse>(async (resolve, reject) => {
      ...
      try {
        ...
        let options: restclient.IRequestOptions = this.createRequestOptions(
          "application/json",
          verData.apiVersion
        );

        let res: restclient.IRestResponse<SessionResponse>;
        res = await this.rest.create<SessionResponse>(
          url,
          sessionRequest,
          options
        );
